...
?><section class="support sect-pad" id="support">
	<div class="container">
		<div> <h2 class="regtitle wow fadeInDown go"><?=$arParams['SECTION_HEADER']?></h2></div>
			...
			?><div class="col-sm-7">
				<ul class="support__ul"><?
					...
					$this->AddDeleteAction($arItem['ID'], $arItem['DELETE_LINK'], CIBlock::GetArrayByID($arItem["IBLOCK_ID"], "ELEMENT_DELETE"), array("CONFIRM" => GetMessage('CT_BCS_ELEMENT_DELETE_CONFIRM')));
						?><li class="support__ul-item wow fadeInDownShort go" data-id="<?=( $k <= $halfOfItems - 1 ) ? $k : $k % $halfOfItems?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
						...
						if( $k == $halfOfItems - 1 ){
							...
							<div class="col-sm-5">
								<ul class="support__ul support__ul_pad" data-sequence="400"><?
						}
					}
				?></ul>
			</div>
			<div class="capabilities__button support_button">
				<a href="/upload/PDF/all.zip" target="_blank" class="btn btn-lblue tahio_btn-lblue wow fadeInUp">Скачать все материалы</a>
			</div>
		</div> <!-- class="row"> -->
	</div> <!-- class="container"> -->
</section>